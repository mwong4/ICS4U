...
an even number: -2. When two numbers are added together to 
produce an even number, both numbers must be even or both must be odd. 
This means if 2c is even, d must also be even. The only prime 
number that is even and exists is 2 since all other numbers that are even 
are divisible by 2 and this not prime. Therefore d must be 2. From this and the 
...
\vspace{0.5cm}
 ...
 \begin{align*}
    ...
    -4 \le \dfrac{x-4}{x+5} & \le 4 && \text{applying identity before splitting}
 \end{align*}
