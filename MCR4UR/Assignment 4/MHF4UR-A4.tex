...
\vspace{0.5cm}
\textbf{Solve question 1 by isolating the equation to one side by performing algebra before factoring}
...
\begin{align*}
    ...
    \log_3((x-7)(x-9)) &= 1 && \text{LOL Product rule, } log_a(xy) = \log_a(x) + \log_a(y)\\
...
than 9. Therefore the solution $x=6$ is inadmissable since is is smaller than 9.
...
\vspace{0.2cm}
Consider the following general formula for exponential functions, where c(t) is the number 
of crows and t is the time in days since Judgement day:

$$c(t) = \text{initial}(1+\text{growth rate})^{\frac{t}{\text{period}}}$$

Where ``initial" is initial population, ``growth rate" is the rate of growth 
per each period and ``period" is the time take for the population to change by 
the growth rate. Let us determine the values required to definte the relationship 
from the data provided in the question.

\begin{center}
    \begin{tabular}{ll}
        Initial Population: & 40000 crows \\
        Rate of growth: & 200 \% increase per period (+2/period) \\
...
or one million, we can solve for t at c(t) = 1000000:
...
difference between log(a) to log(b) and log(b) to log(c) using (3), we 
can derive a new true statement. Remember, an arithmetc sequence 
has a common difference between each term, meaning that term (n) and term (n+1) 
will have the same difference as term (n+1) and term (n+2) and so on:
...
\vspace{-1cm}
$$\therefore \boxed{\text{since LHS = RHS, } \dfrac{1}{1+\log_a(bc)} + \dfrac{1}{1+\log_b(ac)} + \dfrac{1}{1+\log_c(ab)} = 1}$$
...
log function: $log_l(m)=n$. ``m" must be a greater than or equal to 0 since the 
log of a negative value cannot be solved. Therefore, from the question, the two 
equations being logged must be greater than zero:
...
$$\boxed{\therefore a = x-3 \text{ where } x \in \mathbb{R}, x>a+1}$$
